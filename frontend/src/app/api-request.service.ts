...
export class ApiRequestService {
    ...
    get<ReturnJson>(uri: string): Observable<ReturnJson> {
        let jsonHeader = {
        ...
        if (uri.startsWith("/")) {
            return this.http.get<ReturnJson>(this.API_URL + uri, jsonHeader);
        } else {
            return this.http.get<ReturnJson>(this.API_URL + "/" + uri, jsonHeader);
        }
    }
    ...
    postWithJsonHeader(uri: string, body: any): Observable<any> {
        let jsonHeaderWithToken = {
            headers: new HttpHeaders({
                'Content-Type': 'application/json',
                'Authorization': `Token ${this.tokenManager.getToken()}`
            })
        };
        return this.post(uri, body, jsonHeaderWithToken);
    }
